  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <KpiCard label="월 예상매출" value={formatKRW(result.monthlyExpectedSales)} sub={result.interpretations.monthlyExpectedSales} positive={result.monthlyExpectedSales >= 20_000_000} />
        <KpiCard label="월 영업이익" value={formatKRW(result.monthlyOperatingProfit)} sub={result.interpretations.monthlyOperatingProfit} positive={result.monthlyOperatingProfit > 0} />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>손익 계산 상세</CardTitle></CardHeader>
          <CardContent className="space-y-0">
            ...
            <Separator />
            <DetailRow label="월 영업이익" value={formatKRWFull(result.monthlyOperatingProfit)} bold color={result.monthlyOperatingProfit >= 0 ? 'text-emerald-600' : 'text-red-600'} />
            <DetailRow label="영업이익률" value={formatPercent(result.operatingProfitMargin)} color={result.operatingProfitMargin >= 0 ? 'text-emerald-600' : 'text-red-600'} />
          </CardContent>
        </Card>

        <Card>
          <CardHeader><CardTitle>손익분기 분석</CardTitle></CardHeader>
          <CardContent className="space-y-0">
            <DetailRow label="손익분기 일 방문객" value={`${result.breakEvenDailyVisitors.toFixed(0)}명/일`} />
            <DetailRow label="손익분기 일 매출" value={formatKRWFull(result.breakEvenDailySales)} />
            <DetailRow label="현재 대비 여유" value={`${(input.dailyVisitors - result.breakEvenDailyVisitors).toFixed(0)}명`} color={input.dailyVisitors >= result.breakEvenDailyVisitors ? 'text-emerald-600' : 'text-red-600'} />
            <Separator />
            <DetailRow label="초기 투자비용" value={formatKRWFull(input.initialInvestment)} />
            <DetailRow label="투자 회수기간" value={formatMonths(result.paybackPeriod)} bold color={result.paybackPeriod <= 24 ? 'text-emerald-600' : 'text-red-600'} />
            <Separator />
      ...
      {input.locationMemo && (
        <Card>
          <CardHeader><CardTitle>현장 메모</CardTitle></CardHeader>
          <CardContent>
...
function DecisionBadge({ decision, large }: { decision: string; large?: boolean }) {
  ...
  if (decision === '우선검토') return <span className={`${size} rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200 font-semibold`}>{decision}</span>;
  if (decision === '조건부검토') return <span className={`${size} rounded-full bg-amber-50 text-amber-700 border border-amber-200 font-semibold`}>{decision}</span>;
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        <p className={`text-xl font-bold mt-1 tabular ${positive ? 'text-slate-900' : 'text-red-600'}`}>{value}</p>
...
function DetailRow({ label, value, bold, color }: { label: string; value: string; bold?: boolean; color?: string }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
      ...
      <span className={`text-sm tabular ${bold ? 'font-semibold' : ''} ${color ?? 'text-slate-800'}`}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((n) => (
          <div key={n} className={`w-3 h-3 rounded-sm ${n <= score ? 'bg-[#1e3a5f]' : 'bg-slate-100'}`} />
        ))}
      </div>
    </div>
  );
